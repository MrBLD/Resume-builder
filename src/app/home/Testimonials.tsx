...
const TESTIMONIALS = [
  {
    ...
    quote:
      "Students often make silly mistakes on their resume by using inconsistent bullet points or font sizes. NIT-AP Resume’s auto format feature is a great help to ensure consistent format.",
    name: "Dipu Singh",
  ...
  },
  {
    ...
    quote:
      "I used NIT-AP Resume during my last job search and was invited to interview at top tech companies such as Google and Amazon thanks to its slick yet professional resume design.",
    name: "Dipu Singh",
    title: "Software Engineer",
  },
  {
    ...
    quote:
      "Creating a professional resume on NIT-AP Resume is so smooth and easy! It saves me so much time and headache to not deal with google doc template.",
    name: "Dipu Singh",
...
export const Testimonials = ({ children }: { children?: React.ReactNode }) => {
  ...
  return (
    <section className="mx-auto -mt-2 px-8 pb-24">
      <h2 className="mb-8 text-center text-2xl font-bold text-red-500">
        Most{" "}
        {/* <Image src={heartSrc} alt="" className="-mt-1 inline-block w-7" />{" "} */}
        Preffered
      </h2>
        ...
        <div className="relative lg:ml-[-50px]">
          {TESTIMONIALS.map(({ src, quote, name, title }, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`bg-primary absolute max-w-lg rounded-[1.9rem] bg-opacity-30 shadow-md transition-all duration-1000 ease-linear ${className}`}
                onMouseEnter={() => {
                  if (className === "z-10") {
